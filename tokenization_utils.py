...
class PreTrainedTokenizer(object):
    ...
    def tokenize(self, text, **kwargs):
        ...
        tokenized_text = split_on_tokens(None, text)
    ...
    def encode(self, text, add_special_tokens=False):
        ...
        """
        subwords, indices, words = self.tokenize(text)
        subword_ids = self.convert_tokens_to_ids(subwords)
        if add_special_tokens:
            subword_ids = self.add_special_tokens_single_sentence(subword_ids)
            indices = [i + 1 for i in indices]
        return (subword_ids, indices, words)
